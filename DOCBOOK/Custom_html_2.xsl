...
<!-- if 1 then insert data-fancybox into image href
<a href="images/imagename.png" data-fancybox=""> -->

<xsl:param name="fancybox" select="0"/>


<!-- Überschrift 1 ohne vorangestelltes "Kapitel" -->
...
<xsl:template name="process.image">
  ...
  <xsl:variable name="output_filename">
    <xsl:choose>
      ...
      -->
      <xsl:otherwise>
      <!-- replace Backslash ###te -->
	  <xsl:variable name="newtext" select="translate($filename,'\','/')"/>	
  ...
  <xsl:variable name="viewport">
    ...
    <xsl:variable name="fancybox">
	<xsl:value-of select="$fancybox"/>
    </xsl:variable>	
...
-->
  ...
  <xsl:variable name="img">
    <xsl:choose>
	  ...
	  <xsl:choose>
	   <xsl:when test="$fancybox != 0">
	  <!-- <xsl:call-template name="image.fancybox">-->
	   <xsl:element name="a">
	  <xsl:attribute name="href">
        <!-- <xsl:value-of select="$output_filename"/> -->

              <xsl:value-of select="concat($img.src.path, $output_filename)"/>

		
      </xsl:attribute>
	  <xsl:attribute name="data-fancybox">
   	<!--<xsl:text>gallery</xsl:text> --> <!-- Gallery view-->
	</xsl:attribute>
